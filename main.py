...
from read import find_limits, get_data, get_lifetime
# fitting methods
...
from dictionary import get_no_peaks, get_roi, set_params, get_E_n, get_sigma
from properties import calculate_resolution, calculate_activity
...
def main():
    ...
    for detector in detectors:
        ...
        for sample in samples:
            ...
            print("Activity: " + str(calculate_activity(source, get_lifetime(sample))) + " Bq")
            # for each peak 
            for no_peak in range(no_peaks):
                # write ideal paramaters to dictionary for later use
                print(fit_peak(detector, sample, no_peak))
                ...
                sigma = get_sigma(detector, source, no_peak)
                R = calculate_resolution(sigma, E)
